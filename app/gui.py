...
# Lightweight env/key helpers (do this BEFORE importing app.main)
...
def ensure_api_key_or_demo() -> None:
    ...
    In Demo Mode we do nothing (app.main will detect missing key and switch to canned answers).
    """
    _load_saved_env()
    ...
    if use_demo:
        # Stay keyless; app.main will run in Demo Mode automatically
        return
    ...
    while True:
        ...
        # Validate the key by loading app.main and making a tiny test call.
        try:
            import importlib
            ...
            try:
                # Some SDKs raise on invalid key; network errors may also surface here.
...
# Globals filled after importing app.main lazily (post key/demo choice)
MAIN = None  # will hold the imported app.main module
CHUNKS = None
VECS = None
            ...
            if any(k in err_str.lower() for k in ("invalid_api_key", "incorrect api key", "401")):
                ...
                def ui_retry_flow():
                    ...
                    try:
                        ...
                        APP_ENV_PATH.write_text(f"OPENAI_API_KEY={key}\n", encoding="utf-8")
                        os.environ["OPENAI_API_KEY"] = key
                        # Reload app.main so module-level DEMO_MODE and client reinitialize
                        try:
                            importlib.reload(MAIN)
...
def init_index_async(status_label: ttk.Label, ready_cb) -> None:
    ...
    def worker():
        try:
            global CHUNKS, VECS, MAIN
            # Import app.main lazily here (after key/demo choice), then build index
            if MAIN is None:
                import app.main as _main
                MAIN = _main
...
# ----------------- Entrypoint -----------------
def main() -> None:
    # Offer key import or continue in Demo Mode BEFORE importing app.main
    ensure_api_key_or_demo()

    # Now import app.main so it can detect DEMO_MODE correctly
    global MAIN
    if MAIN is None:
        import app.main as _main
        MAIN = _main
